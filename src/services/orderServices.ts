...
export const orderBuyStock = async (
	...
	try {
		...
		if (data.status === 'SUCCESS') {
			return {
				status: 'SUCCESS',
				message: `주문 성공: 구매 가격 ${data.price?.toLocaleString()}원, 수량 ${data.quantity?.toLocaleString()}주`,
			};
		} else if (data.status === 'FAIL - INSUFFICIENT_BALANCE') {
			return { status: 'FAIL', message: '주문 실패: 잔액이 부족합니다.' };
		} else {
			return {
				status: 'FAIL',
				message: '주문 실패: 알 수 없는 오류가 발생했습니다.',
			};
		}
	} catch (error) {
...
export const orderSellStock = async (
	...
	try {
		...
		if (data.status === 'SUCCESS') {
			return {
				status: 'SUCCESS',
				message: `주문 성공: 판매 가격 ${data.price?.toLocaleString()}원, 수량 ${data.quantity?.toLocaleString()}주`,
			};
		} else if (data.status === 'FAIL - INSUFFICIENT_QUANTITY') {
			return { status: 'FAIL', message: '주문 실패: 보유 수량이 부족합니다.' };
		} else if (data.status === 'FAIL - NO_OWNED_STOCK') {
			return {
				status: 'FAIL',
				message: '주문 실패: 보유하지 않은 종목입니다.',
			};
		} else {
			return {
				status: 'FAIL',
